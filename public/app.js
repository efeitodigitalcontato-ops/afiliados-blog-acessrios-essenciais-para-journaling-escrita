...
function showSafiraComicBubble(selector, stepIndex) {
  ...
  if (stepIndex === 1) {
    ...
    text = `Defini a palavra semente como <strong>'${window.comeceRapidoState.selectedMicro ? window.comeceRapidoState.selectedMicro.name : 'seu nicho'}'</strong>. Clique no botão indicado pela seta para buscar ideias de títulos por IA!`;
    position = "bottom";
  } else if (stepIndex === 4) {
    title = "Estrutura Silo";
    text = `Configurei o nicho como <strong>'${window.comeceRapidoState.selectedMicro ? window.comeceRapidoState.selectedMicro.name : 'seu nicho'}'</strong>. Clique no botão de planejamento para interligar os artigos do site automaticamente para o Google!`;
    position = "top";
  } else if (stepIndex === 5) {
    title = "Posição do Site";
    text = `Preenchi o monitoramento com o domínio e a palavra-chave baseados em <strong>'${window.comeceRapidoState.selectedMicro ? window.comeceRapidoState.selectedMicro.name : 'seu nicho'}'</strong>. Clique no botão indicado pela seta para rastrear seu posicionamento!`;
    position = "top";
  } else if (stepIndex === 6) {
    title = "Neto Salva (Backup)";
    text = "Preenchi a descrição do backup. Clique no botão indicado pela seta para criar o ponto de restauração seguro de todo o seu banco de dados!";
    position = "top";
  }
...
function advanceComeceRapidoComic(step) {
  ...
  if (step > currentStep) {
    ...
    if (currentStep === 3 && !window.comeceRapidoState.generatedTitles) {
      const textEl = document.querySelector('.safira-hq-bubble-text');
      if (textEl) {
    ...
    if (currentStep === 4 && !window.comeceRapidoState.structuredSilo) {
      const textEl = document.querySelector('.safira-hq-bubble-text');
      if (textEl) {
        textEl.innerHTML = `<span style="color: #f59e0b; font-weight: bold;">⚠️ Atenção:</span> Você deve clicar no botão <strong>'Planejar e Estruturar SILO'</strong> para interligar os artigos antes de prosseguir!`;
      }
      return;
    }
    if (currentStep === 5 && !window.comeceRapidoState.trackedPosition) {
      const textEl = document.querySelector('.safira-hq-bubble-text');
      if (textEl) {
        textEl.innerHTML = `<span style="color: #f59e0b; font-weight: bold;">⚠️ Atenção:</span> Você deve clicar no botão <strong>'Analisar Posição no Google'</strong> para iniciar o rastreamento antes de prosseguir!`;
      }
      return;
    }
  ...
  if (step === 1) {
    ...
    setTimeout(() => {
      ...
      if (posKw && window.comeceRapidoState.selectedMicro) {
        posKw.value = `melhores ${window.comeceRapidoState.selectedMicro.name.toLowerCase()}`;
      }
      const posUrl = document.getElementById('position-custom-url');
      if (posUrl && window.comeceRapidoState.selectedMicro) {
        const slug = window.comeceRapidoState.selectedMicro.name.toLowerCase()
          .normalize('NFD').replace(/[\u0300-\u036f]/g, '')
          .replace(/[^a-z0-9]+/g, '-')
          .replace(/(^-|-$)/g, '');
        posUrl.value = `${slug}-blog.vercel.app`;
      }
